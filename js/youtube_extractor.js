/**
* Javascript used by Youtube extractor
*/

/**
* @func Get Youtube videos url of page
*/
function getYtVideoUrl(urlPage)
{
	...
	if( Object.keys(json_page).length === 0 ) return null;
	...
	if( encoded_data_list == ',' ) return null;
	...
	for( var encoded_data of encoded_data_list.split(",") ) 
	{
		...
		if( ! decoded_data["size"] ) continue;
		list_url[index]["res"] = get_quality(decoded_data);
		if( final_url["res"] >= decoded_data["res"] ) continue;
