/**
* Javascript used by FORENSIC service
*/
...
var datas_width = ["dqReport", "elaReport", "dwNoiseReport", "blockingReport", "gridsReport", "gridsInversedReport", "medianNoiseReport"];
var datas = datas_width;
...
function display_error(data) 
{
	...
	if( json_lang_translate[global_language]["forensic_error_" + data.status] != undefined ) {
		var error_msg = json_lang_translate[global_language]["forensic_error_" + data.status];
	} else {
		var error_msg = json_lang_translate[global_language]["forensic_error_empty_parameter"];
	}
	...
	$.getJSON(base_url_forensic + "imageforensicsv3/getreport?hash=" + hash, function (data) {
		if( data.status === "completed" ) {
			...
			for( var i = 0; i < datas.length; ++i ) {
				...
				column.id = datas[i];
				var card = create_card( json_lang_translate[global_language]["forensic_title_" + datas[i]], data[datas[i]]["map"], json_lang_translate[global_language]["forensic_card_" + datas[i]] );
			...
			var card = create_card_slider( json_lang_translate[global_language]["forensic_title_ghostReport"], data["ghostReport"]["maps"], json_lang_translate[global_language]["forensic_card_ghostReport"] );
			column.appendChild(card);
			row.appendChild(column);
...
function updateTableLanguageForensic(lang)
{
	...
	if( document.getElementById("forensic-content").style.display === "block" ) {
		...
		// get the card element
		var el = document.getElementById("ghostReport").childNodes[0].childNodes[0];
		// title from card__face--front
