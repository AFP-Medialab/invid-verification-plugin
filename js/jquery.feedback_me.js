...
var fm = (function ($) {
	...
	function emailValid(str) {
		...
		/* after modification */
		return /^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(str);
		/* End modification */
	}
	...
	function appendFeedbackToBody(fm_options) {
		...
		if (fm_options.show_form === true) {
			form_html = '<form class="feedback_me_form">'
				...
				+	'<li> <p style="font-size:12px; position:absolute; bottom:40px;"> <span class="required_asterisk">*</span> : required</p></li>'

				// old:+		'<li>	<button type="submit" onclick="fm.stopPropagation(event);fm.sendFeedback(event,\'' + fm_options.position + '\');" class="feedback_submit ' + bootstrap_btn + '">' + fm_options.submit_label + '</button> </li>'
				+		'<li>	<button type="submit"  class="feedback_submit" style="border: 1px white solid";' + bootstrap_btn + '">' + fm_options.submit_label + '</button> </li>'
				+	'</ul>'
				+	'</form>';
		}
	...
	function sendFeedback(event, position) {
		...
		/* After modification */
		dataArray = '{"text": "' + $fm_content.find(".feedback_message").val().replace(/"/g, '\\"') + "\n"
		 			+ 'email: ' + $fm_content.find(".feedback_email").val() + '"'
					+ ', "username": "' + $fm_content.find(".feedback_name").val() + '"' 
					+ ', "channel": "#general"}';
...
$(document).ready(function(){
    ...
    fm_options = {
        ...
        show_email: true,
        email_required: true,
        email_placeholder: "Please enter your email so we can reply to you",
        message_required: true,
        message_placeholder: "Please report here your use of the plugin: any success you achieve with it or any trouble you may have with an url or a feature…\n" +
	...
	/* Fix inline security error */
    $(".feedback_submit")[0].addEventListener("click", function(event){
    	fm.stopPropagation(event);
    	fm.sendFeedback(event, fm_options.position);
    }, false);

    $('.feedback_trigger')[0].onclick = function(event){
    	fm.stopPropagation(event);
    	fm.triggerAction(event, fm_options.position);
    }
});
